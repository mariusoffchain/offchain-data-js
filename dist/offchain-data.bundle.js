...
var OCMData=(()=>{var R=[{id:"btc-price",title:"Bitcoin Price",subtitle:"BTC/USD daily reference rate",category:"market",type:"line",api:"price",unit:"$",source:"Coin Metrics"},{id:"market-cap",title:"Market Capitalization",subtitle:"Total BTC market value in USD",category:"market",type:"area",api:"marketcap",unit:"$T",source:"Coin Metrics"},{id:"volume",title:"24h Trading Volume",subtitle:"Aggregate exchange spot volume",category:"market",type:"bar",api:"volume",unit:"$B",source:"CoinGecko"},{id:"dominance",title:"Circulating Supply",subtitle:"Total BTC mined to date",category:"market",type:"area",api:"dominance",unit:"M BTC",source:"Coin Metrics"},{id:"hashrate",title:"Network Hashrate",subtitle:"Total computational power",category:"mining",type:"line",api:"hashrate",unit:"EH/s",source:"mempool.space"},{id:"difficulty",title:"Mining Difficulty",subtitle:"Adjusted every 2,016 blocks",category:"mining",type:"line",api:"difficulty",unit:"T",source:"mempool.space"},{id:"block-fees",title:"Block Fees",subtitle:"Average fees per block",category:"mining",type:"bar",api:"fees",unit:"BTC",source:"mempool.space"},{id:"hashprice",title:"Hash Price",subtitle:"Daily revenue per TH/s",category:"mining",type:"line",api:"hashprice",unit:"$/TH",source:"mempool.space"},{id:"active-addresses",title:"Active Addresses",subtitle:"Unique addresses used daily",category:"on-chain",type:"area",api:"addresses",unit:"K",source:"Coin Metrics"},{id:"lth-supply",title:"Miner Revenue",subtitle:"Daily BTC earned by miners (subsidy + fees)",category:"on-chain",type:"area",api:"lth",unit:"BTC",source:"Coin Metrics"},{id:"exchange-balance",title:"Exchange Balances",subtitle:"Total BTC on exchanges",category:"on-chain",type:"line",api:"exchange",unit:"M BTC",source:"Coin Metrics"},{id:"nvt",title:"Daily Transactions",subtitle:"On-chain transaction count per day",category:"on-chain",type:"bar",api:"nvt",unit:"K",source:"Coin Metrics"},{id:"mempool-size",title:"Mempool Size",subtitle:"Pending transactions in vBytes",category:"mempool",type:"area",api:"mempool",unit:"MB",source:"mempool.space"},{id:"fee-rates",title:"Fee Rate Distribution",subtitle:"sat/vB by confirmation priority",category:"mempool",type:"bar",api:"feerates",unit:"sat/vB",source:"mempool.space"},{id:"tx-per-second",title:"Transactions Per Second",subtitle:"On-chain throughput",category:"mempool",type:"line",api:"tps",unit:"tx/s",source:"Coin Metrics"},{id:"block-weight",title:"Average Block Weight",subtitle:"How full are Bitcoin blocks?",category:"mempool",type:"bar",api:"blockweight",unit:"MWU",source:"mempool.space"}];var l={accent:"#F7931A",ink:"#1a1a1a",paper:"#FAF7F1",muted:"rgba(128,128,128,0.5)",grid:"rgba(128,128,128,0.12)",heading:"'Oswald', 'Arial Narrow', sans-serif",body:"'Spectral', Georgia, serif",line:"light-dark(#1a1a1a, #f2f2f2)",lineFill:"light-dark(#1a1a1a, #f2f2f2)",shadow:"light-dark(rgba(0,0,0,0.32), rgba(255,255,255,0.20))",textAdaptive:"light-dark(#1a1a1a, #ffffff)",textAdaptiveInverse:"light-dark(#ffffff, #1a1a1a)",textAdaptiveMuted:"light-dark(rgba(26,26,26,0.55), rgba(255,255,255,0.55))"},re={"1D":24,"1W":7,"1M":30,"3M":90,"1Y":365,ALL:9999};var F="https://ocm-proxy.offchainmedia.workers.dev",se="",Se="https://community-api.coinmetrics.io/v4/timeseries/asset-metrics",_e=3.125,De=144,X={},ee={};function v(e,t,o,n,i){let s=o,r=e,d=Date.now();return Array.from({length:t},(p,c)=>(r=(r*9301+49297)%233280,s+=Math.sin(c*.5+e)*n+i,s=Math.max(o*.3,s),{ts:d-(t-c)*864e5,v:s}))}var ce={price:()=>v(5,365,85e3,4e3,120),marketcap:()=>v(7,365,16e11,8e10,3e9),volume:()=>v(13,365,35e9,8e9,1e8),dominance:()=>v(6,365,19.8,.05,.001),hashrate:()=>v(3,365,600,25,.6),difficulty:()=>v(9,52,80,5,.15),fees:()=>v(11,90,.15,.05,.001),hashprice:()=>v(12,180,.042,.008,1e-4),addresses:()=>v(14,365,9e5,8e4,500),lth:()=>v(13,365,500,30,.5),exchange:()=>v(15,365,2.8,.04,-.001),nvt:()=>v(16,365,6e5,5e4,100),mempool:()=>v(17,90,35,18,.05),feerates:()=>v(18,90,15,10,.02),tps:()=>v(19,90,5.5,1,.002),blockweight:()=>v(20,90,3.7,.2,.001)};function O(e){let t=new Map;for(let{ts:o,v:n}of e){let i=Math.floor(o/864e5),s=t.get(i)||{sum:0,n:0};s.sum+=n,s.n+=1,t.set(i,s)}return[...t.entries()].sort((o,n)=>o[0]-n[0]).map(([o,{sum:n,n:i}])=>({ts:o*864e5,v:n/i}))}async function Pe(){let e=await fetch(`${F}/api/v1/mining/hashrate/all`);if(!e.ok)throw new Error(`hashrate HTTP ${e.status}`);return(await e.json()).hashrates.map(o=>({ts:o.timestamp*1e3,v:o.avgHashrate/1e18}))}async function ze(){let e=await fetch(`${F}/api/v1/mining/difficulty-adjustments/all`);if(!e.ok)throw new Error(`difficulty HTTP ${e.status}`);return(await e.json()).map(([o,,n])=>({ts:o*1e3,v:n/1e12})).sort((o,n)=>o.ts-n.ts)}async function Re(){let e=await fetch(`${F}/api/v1/mining/blocks/fees/all`);if(!e.ok)throw new Error(`fees HTTP ${e.status}`);let t=await e.json();return O(t.map(o=>({ts:o.timestamp*1e3,v:o.avgFees/1e8})))}async function He(){let e=await fetch(`${F}/api/v1/mining/blocks/fee-rates/all`);if(!e.ok)throw new Error(`feerates HTTP ${e.status}`);let t=await e.json();return O(t.map(o=>({ts:o.timestamp*1e3,v:o.avgFee_50})))}async function je(){let e=await fetch(`${F}/api/v1/statistics/all`);if(!e.ok)throw new Error(`mempool HTTP ${e.status}`);let t=await e.json();return O(t.map(o=>({ts:o.added*1e3,v:o.vsizes.reduce((n,i)=>n+i,0)/1e6})))}async function Ie(){let e=await fetch(`${F}/api/v1/mining/blocks/sizes-weights/all`);if(!e.ok)throw new Error(`blockweight HTTP ${e.status}`);let t=await e.json();return O(t.weights.map(o=>({ts:o.timestamp*1e3,v:o.avgWeight/1e6})))}async function T(e){if(ee[e])return ee[e];let t=new Date,n=`${Se}?assets=btc&metrics=${e}&frequency=1d&start_time=${new Date("2010-01-01").toISOString().slice(0,10)}&end_time=${t.toISOString().slice(0,10)}&page_size=10000`,i=await fetch(n);if(!i.ok)throw new Error(`${e} HTTP ${i.status}`);let r=(await i.json()).data.map(d=>({ts:new Date(d.time).getTime(),v:parseFloat(d[e])}));return ee[e]=r,r}async function Ne(){return T("AdrActCnt")}async function qe(){return(await T("TxCnt")).map(({ts:e,v:t})=>({ts:e,v:t/86400}))}async function Oe(){return(await T("SplyExNtv")).map(({ts:e,v:t})=>({ts:e,v:t/1e6}))}async function Ge(){let[{data:e},{data:t},{data:o}]=await Promise.all([E("fees"),E("hashrate"),E("price")]),n=new Map(t.map(s=>[Math.floor(s.ts/864e5),s.v])),i=new Map(o.map(s=>[Math.floor(s.ts/864e5),s.v]));return e.map(({ts:s,v:r})=>{let d=Math.floor(s/864e5),p=n.get(d),c=i.get(d);if(p==null||c==null)return null;let m=De*(_e+r)*c;return{ts:s,v:m/(p*1e6)}}).filter(Boolean)}async function Ue(){return T("PriceUSD")}async function Ve(){return(await T("CapMrktCurUSD")).map(e=>({ts:e.ts,v:e.v/1e12}))}async function We(){let e=se?`&x_cg_demo_api_key=${se}`:"",t=await fetch(`https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=365&interval=daily${e}`);if(!t.ok)throw new Error(`CoinGecko volume HTTP ${t.status}`);return(await t.json()).total_volumes.map(([n,i])=>({ts:n,v:i/1e9}))}async function Ze(){return(await T("SplyCur")).map(e=>({ts:e.ts,v:e.v/1e6}))}async function Ye(){return T("TxCnt")}async function Ke(){let[e,t]=await Promise.all([T("IssTotNtv"),T("FeeTotNtv")]),o=new Map(t.map(n=>[Math.floor(n.ts/864e5),n.v]));return e.map(({ts:n,v:i})=>({ts:n,v:i+(o.get(Math.floor(n/864e5))??0)}))}function Je(){return[{medianFee:2.1,feeRange:[1,1.2,1.3,2,2.7,4,201],nTx:4327,totalFees:25e5,blockSize:2054070},{medianFee:1.4,feeRange:[.5,.6,.7,1,1.5,2.2,150],nTx:6755,totalFees:16e5,blockSize:1997e3},{medianFee:.6,feeRange:[.36,.4,.42,.5,.7,1,45],nTx:5777,totalFees:95e4,blockSize:1584123}]}function Qe(){let e=["F2Pool","Foundry USA","Luxor","AntPool","ViaBTC","\u24C8BTC.com"],t=Date.now();return Array.from({length:6},(o,n)=>({height:957896-n,timestamp:Math.floor((t-n*9.7*6e4)/1e3),tx_count:4362-n*235,size:1561683+n*12e3,extras:{medianFee:1.1+n*.1,feeRange:[.55-n*.02,.7,1,2,3,5,161-n*10],totalFees:1883531-n*9e4,pool:{name:e[n%e.length]}}}))}async function le(){try{let e=await fetch(`${F}/api/v1/fees/mempool-blocks`);if(!e.ok)throw new Error(`mempool-blocks HTTP ${e.status}`);return{data:await e.json(),live:!0}}catch(e){return console.warn('[data] Falling back to mock for "mempool-blocks":',e.message),{data:Je(),live:!1}}}async function de(){try{let e=await fetch(`${F}/api/v1/blocks`);if(!e.ok)throw new Error(`blocks HTTP ${e.status}`);return{data:await e.json(),live:!0}}catch(e){return console.warn('[data] Falling back to mock for "blocks":',e.message),{data:Qe(),live:!1}}}function pe(e){let t=Math.max(0,Math.round((Date.now()-e*1e3)/6e4));return t<1?"just now":t<60?`${t} min ago`:`${Math.round(t/60)}h ago`}var Xe={hashrate:Pe,difficulty:ze,fees:Re,feerates:He,mempool:je,blockweight:Ie,addresses:Ne,tps:qe,exchange:Oe,hashprice:Ge,price:Ue,marketcap:Ve,volume:We,dominance:Ze,nvt:Ye,lth:Ke};async function E(e){if(X[e])return{data:X[e],live:!0};let t=Xe[e];if(t)try{let n=await t();return X[e]=n,{data:n,live:!0}}catch(n){console.warn(`[data] Falling back to mock for "${e}":`,n.message)}return{data:(ce[e]||ce.price)(),live:!1}}function H(e,t){let o=re[t]??e.length;return e.slice(-Math.min(o,e.length))}function N(e,t){return t==="$"?e>=1e3?`$${(e/1e3).toFixed(0)}k`:`$${e.toFixed(0)}`:t==="$T"?`$${e.toFixed(2)}T`:t==="$B"?`$${e.toFixed(1)}B`:t==="EH/s"?`${e.toFixed(1)} EH/s`:t==="%"?`${e.toFixed(1)}%`:t==="M BTC"?`${e.toFixed(2)}M BTC`:t==="K"?`${(e/1e3).toFixed(0)}K`:t==="BTC"?`${e.toFixed(3)} BTC`:t==="T"?`${e.toFixed(1)}T`:e!=null?`${e.toFixed(2)}${t?" "+t:""}`:"\u2014"}function me(e,t){return e==null?"\u2014":t==="$"?`$${Math.round(e).toLocaleString("en-US")}`:t==="BTC"?`${e.toFixed(4)} BTC`:t==="K"?Math.round(e).toLocaleString("en-US"):N(e,t)}function ue(e,t){if(e==null)return"\u2014";if(t==="$")return e>=1e3?`$${Math.round(e).toLocaleString("en-US")}`:e>=1?`$${e.toFixed(2)}`:`$${e.toFixed(5)}`;if(t==="$T")return`$${e.toFixed(3)}T`;if(t==="$B")return`$${e.toFixed(2)}B`;if(t==="EH/s")return`${e.toFixed(2)} EH/s`;if(t==="M BTC")return`${e.toFixed(3)}M BTC`;if(t==="K")return`${Math.round(e).toLocaleString("en-US")}`;if(t==="BTC")return`${e.toFixed(4)} BTC`;if(t==="T")return`${e.toFixed(2)}T`;if(t==="%")return`${e.toFixed(2)}%`;let o=Math.abs(e),n=o>=100?1:o>=10?2:o>=1?3:o>=.1?4:5;return`${e.toFixed(n)}${t?" "+t:""}`}function he(e){return new Date(e).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}function fe(e){let t=e;return()=>(t=(t*9301+49297)%233280,t/233280)}function a(e,t={}){let o=document.createElementNS("http://www.w3.org/2000/svg",e);return Object.entries(t).forEach(([n,i])=>o.setAttribute(n,i)),o}function te(e,t=1,o=1){let n=fe(o),i=e.map(r=>({x:r.x+(n()-.5)*t,y:r.y+(n()-.5)*t}));if(i.length<2)return"";let s=`M${i[0].x.toFixed(2)},${i[0].y.toFixed(2)}`;for(let r=0;r<i.length-1;r++){let d=i[Math.max(0,r-1)],p=i[r],c=i[r+1],m=i[Math.min(i.length-1,r+2)],u=p.x+(c.x-d.x)/6,f=p.y+(c.y-d.y)/6,x=c.x-(m.x-p.x)/6,y=c.y-(m.y-p.y)/6;s+=` C${u.toFixed(2)},${f.toFixed(2)} ${x.toFixed(2)},${y.toFixed(2)} ${c.x.toFixed(2)},${c.y.toFixed(2)}`}return s}function ge(e,t,o){let n=fe(Math.round(e+t)),i=n()*Math.PI,s=[];for(let r=0;r<=Math.PI*2*1.15;r+=.3){let d=o+(n()-.5)*1.5;s.push(`${(e+Math.cos(r+i)*d).toFixed(1)},${(t+Math.sin(r+i)*d).toFixed(1)}`)}return"M"+s.join(" L")}function ye(e,t,o,n){let i=e.map(c=>c.v),s=Math.min(...i),r=Math.max(...i),d=r-s||1;return{points:e.map((c,m)=>({x:t.left+m/(e.length-1)*o,y:t.top+n-(c.v-s)/d*n,d:c})),min:s,max:r,range:d}}var _=680,S=380,h={top:52,right:20,bottom:46,left:52},A=_-h.left-h.right,w=S-h.top-h.bottom,et=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],G=5,tt=0;function U(e,t,o,n){e.innerHTML="";let i=H(t,n);if(!i.length)return;let{points:s,min:r,max:d,range:p}=ye(i,h,A,w),c=a("svg",{width:"100%",viewBox:`0 0 ${_} ${S}`,style:"display:block"}),m=a("defs"),u=a("filter",{id:"ocm-rough",x:"-5%",y:"-5%",width:"110%",height:"110%"}),f=a("feTurbulence",{type:"fractalNoise",baseFrequency:"0.012",numOctaves:"2",seed:"4",result:"n"}),x=a("feDisplacementMap",{in:"SourceGraphic",in2:"n",scale:"2",xChannelSelector:"R",yChannelSelector:"G"});u.appendChild(f),u.appendChild(x),m.appendChild(u);let y=a("filter",{id:"ocm-drop",x:"-20%",y:"-20%",width:"140%",height:"140%"}),C=a("feGaussianBlur",{in:"SourceAlpha",stdDeviation:"3",result:"blur"}),L=a("feOffset",{in:"blur",dx:"2",dy:"4",result:"off"}),M=a("feFlood",{style:`flood-color: ${l.shadow}`,result:"floodColor"}),K=a("feComposite",{in:"floodColor",in2:"off",operator:"in",result:"sh"}),j=a("feMerge");j.appendChild(a("feMergeNode",{in:"sh"})),j.appendChild(a("feMergeNode",{in:"SourceGraphic"})),y.appendChild(C),y.appendChild(L),y.appendChild(M),y.appendChild(K),y.appendChild(j),m.appendChild(y);let k=a("linearGradient",{id:"ocm-vol",x1:"0",y1:"0",x2:"0",y2:"1"});k.appendChild(a("stop",{offset:"0%",style:`stop-color: ${l.lineFill}`,"stop-opacity":"0.22"})),k.appendChild(a("stop",{offset:"100%",style:`stop-color: ${l.lineFill}`,"stop-opacity":"0.02"})),m.appendChild(k);let P=a("linearGradient",{id:"ocm-ridge",x1:"0",y1:"0",x2:"0",y2:"1"});P.appendChild(a("stop",{offset:"0%","stop-color":"#fff","stop-opacity":"0.30"})),P.appendChild(a("stop",{offset:"15%","stop-color":"#fff","stop-opacity":"0"})),m.appendChild(P);let z=a("clipPath",{id:"ocm-clip"});z.appendChild(a("rect",{x:h.left,y:h.top,width:A,height:w})),m.appendChild(z),c.appendChild(m),c.appendChild(a("rect",{width:_,height:S,fill:"transparent"}));let B=p/G,q=b=>{let $=Math.abs(b);return $>=1e9?`${(b/1e9).toFixed(p/1e9<5?1:0)}B`:$>=1e6?`${(b/1e6).toFixed(p/1e6<5?1:0)}M`:$>=1e3?`${(b/1e3).toFixed(p/1e3<5?1:0)}k`:B>=10?b.toFixed(0):B>=1?b.toFixed(1):B>=.1?b.toFixed(2):B>=.01?b.toFixed(3):b.toFixed(4)};for(let b=0;b<=G;b++){let $=h.top+b/G*w,Q=d-b/G*p;c.appendChild(a("line",{x1:h.left,y1:$,x2:h.left+A,y2:$,stroke:l.grid,"stroke-width":"0.7","stroke-linecap":"round"}));let I=a("text",{x:h.left-8,y:$+4,"text-anchor":"end","font-family":l.body,"font-size":"12",fill:"rgba(128,128,128,0.65)","font-style":"italic"});I.textContent=q(Q),c.appendChild(I)}c.appendChild(a("line",{x1:h.left,y1:h.top+w,x2:h.left+A,y2:h.top+w,stroke:"rgba(128,128,128,0.28)","stroke-width":"1","stroke-linecap":"round"}));let Le=(i.length>1?i[i.length-1].ts-i[0].ts:0)>400*864e5;[0,.25,.5,.75,1].forEach(b=>{let $=Math.round(b*(i.length-1)),Q=h.left+b*A,I=new Date(i[$]?.ts||Date.now()),ae=a("text",{x:Q,y:h.top+w+18,"text-anchor":"middle","font-family":l.body,"font-size":"12",fill:"rgba(128,128,128,0.6)","font-style":"italic"});ae.textContent=Le?I.getFullYear():et[I.getMonth()],c.appendChild(ae)}),o.type==="bar"?ot(c,m,i,r,p):nt(c,s,t,o,i),it(c,m),c.appendChild(a("line",{x1:0,y1:S-14,x2:_,y2:S-14,stroke:l.ink,"stroke-width":"0.5",opacity:"0.3"}));let ne=a("text",{x:h.left,y:S-4,"font-family":l.body,"font-size":"9",fill:"rgba(128,128,128,0.4)","font-style":"italic"});ne.textContent=`Source: ${o.source}`,c.appendChild(ne);let ie=a("text",{x:_-h.right,y:S-4,"text-anchor":"end","font-family":l.heading,"font-size":"8",fill:"rgba(128,128,128,0.35)","letter-spacing":"0.1em"});ie.textContent="OFFCHAIN.MEDIA/DATA",c.appendChild(ie),e.appendChild(c)}function ot(e,t,o,n,i){let r=(A-3*(o.length-1))/Math.max(o.length,1),d=a("g",{filter:"url(#ocm-rough)"});o.forEach((p,c)=>{let m=(p.v-n)/i*w,u=h.left+c*(r+3),f=h.top+w-m,x=c===o.length-1,y=x?l.accent:l.line,C=`ocm-bv-${c}`,L=a("linearGradient",{id:C,x1:"0",y1:"0",x2:"0",y2:"1"});L.appendChild(a("stop",{offset:"0%",style:`stop-color: ${y}`,"stop-opacity":x?"1":"0.85"})),L.appendChild(a("stop",{offset:"100%",style:`stop-color: ${y}`,"stop-opacity":x?"0.75":"0.6"})),t.appendChild(L);let M=a("g",{filter:"url(#ocm-drop)"});M.appendChild(a("rect",{x:u,y:f,width:r,height:m,rx:"1.5",fill:`url(#${C})`})),M.appendChild(a("rect",{x:u,y:f,width:r,height:Math.min(m,16),rx:"1.5",fill:"rgba(255,255,255,0.28)"})),d.appendChild(M)}),e.appendChild(d)}function nt(e,t,o,n,i){let s=te(t,1,3),r=`${s} L${t[t.length-1].x},${h.top+w} L${t[0].x},${h.top+w} Z`,d=a("g",{"clip-path":"url(#ocm-clip)"});d.appendChild(a("path",{d:r,fill:"url(#ocm-vol)"})),d.appendChild(a("path",{d:r,fill:"url(#ocm-ridge)"})),e.appendChild(d);let p=a("g",{filter:"url(#ocm-rough)","clip-path":"url(#ocm-clip)"});p.appendChild(a("path",{d:s,fill:"none",style:`stroke: ${l.line}`,"stroke-width":"2.2","stroke-linecap":"round","stroke-linejoin":"round",filter:"url(#ocm-drop)"})),t.length>12&&p.appendChild(a("path",{d:te(t.slice(-12),1,4),fill:"none",stroke:l.accent,"stroke-width":"2.5","stroke-linecap":"round",filter:"url(#ocm-drop)"})),e.appendChild(p);let c=t[t.length-1];c&&(e.appendChild(a("circle",{cx:c.x,cy:c.y,r:"4",fill:l.accent})),e.appendChild(a("path",{d:ge(c.x,c.y,9),fill:"none",stroke:l.accent,"stroke-width":"1.2",opacity:"0.6","stroke-linecap":"round"})));let m=a("line",{x1:0,y1:h.top,x2:0,y2:h.top+w,stroke:l.accent,"stroke-width":"0.7","stroke-dasharray":"3,3",opacity:"0","pointer-events":"none"});e.appendChild(m);let u=a("g",{opacity:"0","pointer-events":"none"}),f=a("rect",{x:0,y:0,width:130,height:42,rx:"2",fill:l.ink}),x=a("text",{x:10,y:14,"font-family":l.body,"font-size":"10",fill:"rgba(255,255,255,0.55)","font-style":"italic"}),y=a("text",{x:10,y:32,"font-family":l.heading,"font-size":"14","font-weight":"600",fill:"#fff"});u.appendChild(f),u.appendChild(x),u.appendChild(y),e.appendChild(u);let C=a("rect",{x:h.left,y:h.top,width:A,height:w,fill:"transparent",style:"cursor:crosshair"});C.addEventListener("mousemove",L=>{let M=e.getBoundingClientRect(),K=_/M.width,j=(L.clientX-M.left)*K,k=t[0],P=1/0;t.forEach(q=>{let J=Math.abs(q.x-j);J<P&&(P=J,k=q)}),m.setAttribute("x1",k.x),m.setAttribute("x2",k.x),m.setAttribute("opacity","0.5");let z=Math.min(k.x+8,_-140),B=Math.max(k.y-52,h.top);f.setAttribute("x",z),f.setAttribute("y",B),x.setAttribute("x",z+10),x.setAttribute("y",B+14),y.setAttribute("x",z+10),y.setAttribute("y",B+32),x.textContent=he(k.d.ts),y.textContent=ue(k.d.v,n.unit),u.setAttribute("opacity","1")}),C.addEventListener("mouseleave",()=>{m.setAttribute("opacity","0"),u.setAttribute("opacity","0")}),e.appendChild(C)}function it(e,t){let o=`ocmwm${tt++}`,n=h.left+A/2,i=h.top+w/2,s=160/956,r=n-s*862,d=i-s*415,p=(x,y)=>{let C=a("clipPath",{id:o+x});C.appendChild(a("path",{d:y})),t.appendChild(C)};p("r","M726 201 L1077 201 L1077 629 L726 629Z"),p("l","M384 201 L999 201 L999 629 L384 629Z"),p("b","M934.7 537.1 L1015.5 537.1 L1015.5 628.2 L934.7 628.2Z");let c=a("g",{transform:`translate(${r.toFixed(1)},${d.toFixed(1)}) scale(${s.toFixed(4)})`,style:`fill: ${l.line}`,opacity:"0.06"}),m=a("g",{"clip-path":`url(#${o}r)`});m.appendChild(a("path",{d:"M1340.3 414.7C1340.4 296.8 1244.8 201.2 1126.9 201.3L958.4 201.4C965.7 207.3 972.8 213.7 979.6 220.5C1000.9 241.8 1018.2 265.9 1031.3 292.1L1126.8 292C1159.6 292 1190.4 304.8 1213.6 328C1236.8 351.1 1249.6 382 1249.5 414.8C1249.5 447.6 1236.7 478.4 1213.5 501.6C1190.3 524.8 1159.4 537.6 1126.7 537.6L939.8 537.7C907 537.8 876.2 525 852.9 501.8C829.8 478.6 817 447.8 817.1 415C817 399.1 820.1 383.6 825.9 369.2C814.7 359.1 800.3 353.6 785.1 353.6L735.3 353.7C729.5 373.1 726.3 393.8 726.2 415.1C726.2 533 821.8 628.6 939.7 628.5L1126.6 628.4C1244.5 628.3 1340.2 532.7 1340.3 414.7Z"})),c.appendChild(m);let u=a("g",{"clip-path":`url(#${o}l)`});u.appendChild(a("path",{d:"M746.3 610.5C725 589.2 707.5 564.8 694.2 537.9L598.1 537.9C565.3 538 534.4 525.2 511.3 502.1C488.1 478.9 475.3 448.1 475.4 415.2C475.4 382.4 488.2 351.6 511.4 328.4C534.6 305.2 565.4 292.4 598.3 292.4L785.1 292.2C817.9 292.3 848.7 305 871.9 328.2C895.1 351.4 907.8 382.2 907.8 415C907.8 430.9 904.8 446.4 899 460.8C910.2 471 924.6 476.5 939.9 476.4L989.6 476.4C995.4 456.9 998.6 436.3 998.6 414.9C998.7 297 903.1 201.4 785.2 201.5L598.3 201.6C480.3 201.7 384.6 297.3 384.6 415.3C384.5 533.3 480 628.8 598 628.8L766.2 628.6C759.4 623 752.7 616.9 746.3 610.5Z"})),c.appendChild(u),c.appendChild(a("path",{d:"M771.5 430C796.8 542.8 866.9 593.7 982.1 582.7",style:`fill:none;stroke:${l.line};stroke-width:50;stroke-linecap:butt;stroke-miterlimit:4`}));let f=a("g",{"clip-path":`url(#${o}b)`});f.appendChild(a("rect",{x:"934.7",y:"537.1",width:"80.8",height:"91.2"})),c.appendChild(f),e.appendChild(c)}function xe(e){let t=document.querySelector(".ocm-gallery-view"),o=document.querySelectorAll(".ocm-data-filters .ocm-filter-btn");if(!t||!o.length)return null;let n=document.createElement("nav");n.className="ocm-sidebar";let i=[...o].map(s=>{let r=document.createElement("div");return r.className="ocm-sidebar-item",r.textContent=s.textContent.trim(),r.dataset.filter=s.dataset.filter,s.classList.contains("ocm-filter-active")&&r.classList.add("ocm-sidebar-active"),r.addEventListener("click",()=>{i.forEach(d=>d.classList.remove("ocm-sidebar-active")),r.classList.add("ocm-sidebar-active"),e(r.dataset.filter)}),n.appendChild(r),r});return t.insertBefore(n,t.firstChild),n}var be=!1;function ve(){if(be)return;be=!0;let e=document.createElement("style");e.textContent=`
  ...
  `,o.appendChild(d),o}var V=null,W="1Y",Z=document.querySelector(".ocm-gallery-view"),Y=document.querySelector(".ocm-detail-view");function yt(){Z&&(Z.style.display=""),Y&&(Y.style.display="none"),V=null,window.scrollTo({top:0,behavior:"smooth"})}async function Ae(e){V=e,W="1Y",Z&&(Z.style.display="none"),Y&&(Y.style.display=""),window.scrollTo({top:0,behavior:"smooth"}),await oe(e,W)}function xt(){document.querySelectorAll(".ocm-period-btn").forEach(e=>{e.addEventListener("click",async()=>{W=e.dataset.period,V&&await oe(V,W)})})}function bt(){let e=document.querySelector(".ocm-back-btn");e&&e.addEventListener("click",yt)}document.addEventListener("ocm:openChart",e=>Ae(e.detail));async function Fe(){xt(),bt(),Ee(),await Ce(Ae)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Fe):Fe();})();
